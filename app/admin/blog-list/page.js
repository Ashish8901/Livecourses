...
async function getBlogs({ limit, pageNumber, keyword }) {
  console.log("keywordblog :>> ", keyword);
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/api/blogs`,
      {
        params: { limit, page: pageNumber, keyword },
      }
    );
...
export default async function Page({ searchParams }) {
  console.log("searchParamsblog :>> ", searchParams);
        ...
        <div className="h-full mb-10 md:ml-64">
          <div className="mx-auto w-full px-4 py-8 sm:px-8">
            <div className="overflow-y-hidden">
                          ...
                          <tr className=" hover:bg-gray-100 text-gray-700 border-b dark:border-gray-700">
                            <td className="px-4 py-3">
                              <p className="whitespace-no-wrap">
                                {" "}
                                {(blogsData.page - 1) * blogsData.limit +
                                  index +
                                  1}
                              </p>
                            </td>
                            ...
                            <td className="px-4 py-3 text-sm">
                              <p className="whitespace-no-wrap">
                                <Link href={`/admin/blog-list/${item.id}`}>
              ...
              <BlogPagination blogsData={blogsData} />
            </div>
          </div>
        </div>
      </div>
